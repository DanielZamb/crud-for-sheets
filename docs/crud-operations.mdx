...
```javascript
db.create(tableConfig.tableName, newRecord, Object.keys(tableConfig.fields));
```
...
but for the love of Linus Torvalds, _PUT THEM IN THE DB CONTEXT_ (or as the message would say _schema_ 🧏‍♂️)
...
function addEmployees(newEmployees) {
  ...
  const result = db.create(
    employeeTableConfig.tableName,
    newEmployees,
    Object.keys(employeeTableConfig.fields)
  );
  // Method 2
  const result = db.create("EMPLOYEES", newEmployees, [
    "name",
    "age",
    "position",
    "employed",
...
### Defaults behavior on create

- Defaults (including `default: "now"` for dates) are applied BEFORE validation and type checks.
- If a schema field has a default, it does not need to be present in `keyOrder`.
- If you set `treatNullAsMissing` or `treatEmptyStringAsMissing`, `null`/`""` will be replaced by the default (if defined).
...
### Defaults behavior on update

- Defaults are applied to missing values before validation.
- Use `treatNullAsMissing` / `treatEmptyStringAsMissing` on a field to opt-in to replacing `null`/`""` with the schema default on updates as well.
...
Remove records from your tables. Deleted records are _moved_ to a history table for tracking purposes.
...
function deleteEmployee(id) {
  ...
  //method 1
  const deleteResult = db.remove(
    employeeTableConfig.tableName,
    employeeTableConfig.historyTablename,
    id
  );
...
function deleteEmployeeCascade(id) {
  ...
  const deleteResult = db.removeWithCascade(
    "EMPLOYEES",
    "DELETED_EMPLOYEES",
    id
  );
